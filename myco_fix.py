...
RESULTS_FILE = "mininet_final_results.csv"
...
def run_test():
    ...
    info( '\n[SETUP] Building Topology with DNAT capability...\n' )
    net = Mininet(controller=Controller, link=TCLink, switch=OVSKernelSwitch)
    net.addController('c0')
    s1 = net.addSwitch('s1')
    
    # h1 = Client, h2 = Normal Server, h3 = Sandbox
    h1 = net.addHost('h1', ip='10.0.0.1')
    h2 = net.addHost('h2', ip='10.0.0.2')
    h3 = net.addHost('h3', ip='10.0.0.3')
    
    # Get MAC addresses for rewriting rules
    h2_mac = h2.MAC()
    h3_mac = h3.MAC()
    ...
    net.start()
    net.pingAll() # Important: Populate ARP tables
    ...
    for strat in strategies:
        ...
        if strat == "Myco-Scout":
            ...
            # Action: Redirect to Sandbox (Port 3)
            # CRITICAL FIX: Rewrite DST MAC and DST IP to match h3 so kernel accepts it
            action = f"mod_dl_dst:{h3_mac},mod_nw_dst:10.0.0.3,output:3"
            s1.dpctl('add-flow', f'priority=1000,in_port=1,actions={action}')
            
        elif strat == "Myco-Swap":
            # Action: Force forward to Normal Server (Port 2)
            # We explicitly define flow to prove continuity
        ...
        target_mac = h3_mac if strat == "Myco-Box" else h2_mac
        target_ip = "10.0.0.3" if strat == "Myco-Box" else "10.0.0.2"
        ...
        # Start Client (Always aiming at h2 initially)
