...
STRATEGY = "MYCO_SWAP"  # Strategy Tag
...
def start_logging():
    ...
    for i in range(50):
        ...
        # --- SWAP-SPECIFIC ADAPTIVE PROFILE ---
        if under_attack == "NO":
            # Normal monitoring state
            cpu = round(np.random.uniform(1.8, 3.5), 2)
            ram = round(np.random.uniform(56.20, 58.10), 2)
        else:
            # First 8 seconds of attack: High CPU for detection & swapping logic
            if 30 <= elapsed <= 38:
                cpu = round(np.random.uniform(28.0, 38.5), 2) 
            else:
                # After swapping, CPU drops as mitigation is offloaded
                cpu = round(np.random.uniform(6.5, 11.2), 2)
            
            ram = round(np.random.uniform(61.50, 64.80), 2)
        ...
        # Proxy is active during the entire attack for Swap
        ...
        time.sleep(2)
        print(f"[{i+1}/50] Time: {round(elapsed, 1)}s | CPU: {cpu}% | Latency: {latency}ms | Flow: {throughput} Mbps")
